...
const STATIC_CELL_DEPS_JSON_URL = 'https://typeid-contract-cell-deps.rgbpp.com/deployment/cell-deps.json';

const CELL_DEPS_JSON_URL = 'https://typeid-contract-cell-deps.rgbpp.com/api/cell-deps';

const request = (url: string) => axios.get(url, { timeout: 10000 });

const fetchCellDepsJsonFromStaticSource = async () => {
  try {
    const response = await Promise.any([request(STATIC_CELL_DEPS_JSON_URL), request(GITHUB_CELL_DEPS_JSON_URL)]);
...
export const fetchCellDepsJson = async () => {
  try {
    const response = await request(CELL_DEPS_JSON_URL);
...
const STATIC_COMPATIBLE_XUDT_URL = 'https://typeid-contract-cell-deps.rgbpp.com/compatible-udt.json';
const GITHUB_STATIC_COMPATIBLE_XUDT_URL =
  'https://raw.githubusercontent.com/RGBPlusPlus/typeid-contract-cell-deps/main/compatible-udt.json';
...
export class CompatibleXUDTRegistry {
  ...
  private static xudtUrl = STATIC_COMPATIBLE_XUDT_URL;
   ...
   * URL (`STATIC_CELL_DEPS_JSON_URL`) will be used.
   * @returns A promise that resolves when the cache has been refreshed.
   */
  static async refreshCache(url?: string): Promise<void> {
    this.xudtUrl = url ?? STATIC_COMPATIBLE_XUDT_URL;
    const isExternal = url !== STATIC_COMPATIBLE_XUDT_URL && url !== GITHUB_STATIC_COMPATIBLE_XUDT_URL;
    try {
