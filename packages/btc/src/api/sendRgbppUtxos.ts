import {
  ...
  checkCkbTxInputsCapacitySufficientCCC,
} from '@rgbpp-sdk/ckb';
...
export interface BuildRgbppUtxosProps {
...
export async function buildRgbppUtxos(props: BuildRgbppUtxosProps): Promise<bitcoin.Psbt> {
  const { builder } = await createSendRgbppUtxosBuilderCCC(props);
  return builder.toPsbt();
}

export async function createSendRgbppUtxosBuilderCCC(props: BuildRgbppUtxosProps): Promise<{
  builder: TxBuilder;
  fee: number;
  feeRate: number;
  changeIndex: number;
  ...
  const ckbPartialTx = props.partialCkbTx;
  const config = networkTypeToConfig(props.source.networkType);
  const isCkbMainnet = props.source.networkType === NetworkType.MAINNET;

  const ckbSigner = props.ckbSigner;
  const liveCells = await Promise.all(
    ckbPartialTx.inputs.map((input) => ckbSigner.client.getCellLive(input.previousOutput!)),
  );
  // ! TODO: TMP WORKAROUND, FIXME
  const rgbppLockArgsList = liveCells.map((cell) =>
    cell &&
    isRgbppLockCell(
      {
        ...cell.cellOutput,
      ...
      ? unpackRgbppLockArgs(cell.cellOutput.lock.args)
      : undefined,
  );
  ...
  // Handle and check inputs
  for (let i = 0; i < ckbPartialTx.inputs.length; i++) {
    const rgbppLockArgs = rgbppLockArgsList[i];
  ...
  // Handle and check outputs
  for (let i = 0; i < ckbPartialTx.outputs.length; i++) {
    // ! TODO: TMP WORKAROUND, FIXME
    const ckbOutput = {
      ...ckbPartialTx.outputs[i],
      capacity: ckbPartialTx.outputs[i].capacity.toString(),
    };
    const isRgbppLock = isRgbppLockCell(ckbOutput, isCkbMainnet);
    const isBtcTimeLock = isBtcTimeLockCell(ckbOutput, isCkbMainnet);
  ...
  // ! TODO: TMP WORKAROUND, FIXME
  const calculatedCommitment = calculateCommitment({
    inputs: ckbPartialTx.inputs.map((input) => ({
      ...input,
      previousOutput: {
    ...
    outputs: ckbPartialTx.outputs
      .map((output) => ({
        ...output,
        capacity: `0x${output.capacity.toString(16)}`,
      }))
      .slice(0, lastCkbTypeOutputIndex + 1),
    outputsData: ckbPartialTx.outputsData.slice(0, lastCkbTypeOutputIndex + 1),
  });
  if (props.commitment !== calculatedCommitment) {
    throw new TxBuildError(ErrorCodes.CKB_UNMATCHED_COMMITMENT);
  }
...
async function getMergedBtcOutputsCCC(btcOutputs: InitOutput[], props: BuildRgbppUtxosProps): Promise<InitOutput[]> {
  const merged: InitOutput[] = [];
  ...
  const isNeedPaymasterOutput = await (async () => {
    ...
    const isInputsSufficient = await checkCkbTxInputsCapacitySufficientCCC(props.partialCkbTx, props.ckbSigner);
    return !isInputsSufficient;
  })();
  if (paymaster && isNeedPaymasterOutput) {
    merged.push({
